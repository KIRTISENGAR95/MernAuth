...
const EmailVerify = () => {
  ...
  const onSubmitHandler = async (e) =>{
    try {
      ...
      if(data.success){
        console.log('Verification successful, updating user data...');
        toast.success(data.message)
        await getUserData();  // Wait for user data to be updated
        console.log('User data updated, navigating...');
        navigate('/')
      }else{
    ...
    <div className='flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400'>
      <img onClick={()=>navigate('/')} src={assets.logo} alt="Logo" className='absolute left-5 sm:left-20 top-5 w-28 sm:w-32 cursor-pointer'/>
       <form onSubmit={onSubmitHandler} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <button className='w-full py-3 bg-3 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full'>Verify email</button>
       </form>
    </div>
  )
}
