...
export const register = async (req,res)=>{
    const {name,email,password}=req.body;
    console.log('Registration request:', { name, email }); // Log registration data
...
export const sendVerifyOtp = async (req,res)=>{
    try {
        ...
        const mailOption={
            ...
            to: user.email,
            subject: ' otp send',
            // text: `official otp: ${otp}`,
            html: EMAIL_VERIFY_TEMPLATE.replace("{{otp}}",otp).replace("{{email}}",user.email)
        }
        await transporter.sendMail(mailOption);
        res.json({success: true, message: 'Verification OTP is on email'}); 
...
export const verifyEmail = async (req, res) => {
    ...
    try {
        const user = await userModel.findById(userId);
        console.log('Found user:', { id: user?._id, verifyOtp: user?.verifyOtp, expires: user?.verifyOtpExpireAt });
...
export const sendResetOtp=async(req,res)=>{
    ...
    try {
        ...
        const mailOption={
            ...
            // use this OTP to proceed with it`,
...
export const resetPassword=async(req,res)=>{
    ...
    try {
        ...
        return res.json({success:true,message:'Password has reset sucessfully'});
    } catch (error) {
        return res.json({success:false,message:error.message});
    }
}